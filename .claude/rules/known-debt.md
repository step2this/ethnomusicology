...
| `duration_ms` type inconsistency across pipeline | Critic 7b H2 | Low | Model uses `i32`, upsert accepts `i64`, route response is `i64`. Import casts `u64 as i64`. Standardize on one type. |
