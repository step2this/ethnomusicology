use crate::services::camelot::{self, CamelotKey, ScoreBreakdown};
...
pub fn arrange_tracks(tracks: &[ArrangementTrack]) -> ArrangementResult {
    if tracks.is_empty() {
    ...
    for (pos, &idx) in order.iter().enumerate() {
        if let Some(e) = tracks[idx].energy {
            energy_scores.push(camelot::energy_arc_score(e, pos, total));
        } else {
...
mod tests {
    ...
    fn test_single_track() {
        ...
        let result = arrange_tracks(&tracks);
    ...
    fn test_deterministic() {
        ...
        let first_result = arrange_tracks(&tracks);
        for _ in 0..10 {
            let result = arrange_tracks(&tracks);
            assert_eq!(
